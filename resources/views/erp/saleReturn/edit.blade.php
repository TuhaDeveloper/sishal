        ...
        <div class="container-fluid px-4 py-3 bg-white border-bottom">
            ...
            <form id="saleReturnForm" action="{{ route('saleReturn.update', $saleReturn->id) }}" method="POST">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="customer_id" id="customer_id" class="form-select" required>
                            ...
                            @foreach($customers as $customer)
                                <option value="{{ $customer->id }}" {{ $saleReturn->customer_id == $customer->id ? 'selected' : '' }}>{{ $customer->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-6">
                        ...
                        <select name="pos_sale_id" id="pos_sale_id" class="form-select" required>
                            ...
                            @foreach($posSales as $pos)
                                <option value="{{ $pos->id }}" {{ $saleReturn->pos_sale_id == $pos->id ? 'selected' : '' }}>POS #{{ $pos->id }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="row mb-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="refund_type" id="refund_type" class="form-select" required>
                            <option value="none" {{ $saleReturn->refund_type == 'none' ? 'selected' : '' }}>None</option>
                            <option value="cash" {{ $saleReturn->refund_type == 'cash' ? 'selected' : '' }}>Cash</option>
                            <option value="bank" {{ $saleReturn->refund_type == 'bank' ? 'selected' : '' }}>Bank</option>
                            <option value="credit" {{ $saleReturn->refund_type == 'credit' ? 'selected' : '' }}>Credit</option>
                        </select>
                    </div>
                    <div class="col-md-4">
                        ...
                        <select name="return_to_id" id="return_to_id" class="form-select mt-2" style="display:{{ $saleReturn->return_to_type ? '' : 'none' }};" required>
                            <option value="">Select Location</option>
                            @if($saleReturn->return_to_type == 'branch')
                                @foreach($branches as $branch)
                                    <option value="{{ $branch->id }}" {{ $saleReturn->return_to_id == $branch->id ? 'selected' : '' }}>{{ $branch->name }}</option>
                                @endforeach
                            @elseif($saleReturn->return_to_type == 'warehouse')
                                @foreach($warehouses as $warehouse)
                                    <option value="{{ $warehouse->id }}" {{ $saleReturn->return_to_id == $warehouse->id ? 'selected' : '' }}>{{ $warehouse->name }}</option>
                                @endforeach
                            @elseif($saleReturn->return_to_type == 'employee' && $saleReturn->return_to_id)
                                <option value="{{ $saleReturn->return_to_id }}" selected>{{ $saleReturn->employee ? ($saleReturn->employee->user->first_name . ' ' . $saleReturn->employee->user->last_name) : 'Selected Employee' }}</option>
                            @endif
                        </select>
                    </div>
                </div>
                <div class="mb-3">
                    ...
                    <input type="text" name="reason" id="reason" class="form-control" value="{{ $saleReturn->reason }}">
                </div>
                <div class="mb-3">
                    <label for="notes" class="form-label">Notes</label>
                    <textarea name="notes" id="notes" class="form-control" rows="2">{{ $saleReturn->notes }}</textarea>
                </div>
                <div class="mb-3">
